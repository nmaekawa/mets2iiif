...
# to remove django dependency on settings
class settings(object):

    IIIF = {
        'context': 'http://iiif.io/api/image/1/context.json',
        'imageUriBase': os.environ.get(
            'IMAGE_URI_BASE', 'http://ids.lib.harvard.edu/ids/iiif/'),
        ...
        'manifestUriTmpl': 'http://{}/manifests/',
        'profileLevel':
            'http://library.stanford.edu/iiif/image-api/1.1/conformance.html#level1',
        'serviceBase': os.environ.get(
            'SERVICE_BASE', 'http://ids.lib.harvard.edu/ids/iiif/'),
    ...
    METS_FOOTER = ''
    SOLR_BASE = 'solr_base'
    SOLR_QUERY_PREFIX = 'solr_query_prefix'
    SOLR_FILE_QUERY = 'solr_file_query'
    SOLR_CURSORMARK = 'solr_cursormark'
...
IIIF_MANIFEST_HOST = environ.get("IIIF_MANIFEST_HOST", "iiif.lib.harvard.edu")
...
def get_display_image(fids):
        ...
        versions = functools.reduce(proc_fid, fids, {})
        display_img = None
...
def get_hollis_viewingDirection(hollisID):
    # mock consult to hollis about viewingDirection
    '''
    try:
        response = urllib_request.urlopen(HOLLIS_API_URL+hollisID).read()
    except:
        logger.debug("HOLLIS lookup failed for Hollis id: " + hollisID)
    else:
        response_data = re.sub(r'(?i)encoding=[\'\"]utf\-8[\'\"]','', response)
...
def get_image_dimension_from_solr(document_id):
    widths = ['2048']
    heights = ['2048']
    ...
    '''
                ...
                while not_paged:
                  try:
                        ...
                        response = requests.get(metadata_url, cookies=cookie)
                  except urllib_request.HTTPError as err:
                        not_paged = False
                        msg = "The document ID {} does not exist in solr index".format(document_id)
                  ...
                  #for md in md_json:
                        if (('object_huldrsadmin_accessFlag_string' in md) and ('file_id_num' not in md)):
                                ...
                                if access_flag == "N":
                                  msg = "Document ID {} is not intended for delivery and cannot be indexed.".format(document_id)
                                  logger.debug(msg)
                                  return False
                                else:
                                  continue
                        if (('file_mix_imageHeight_num' in md) and ('file_mix_imageWidth_num' in md)):
                                ...
                                if (file_ext == 'jp2' or file_ext == 'tif' or file_ext == 'jpg' or file_ext == 'gif'):
                                  ...
                                  if 'object_huldrsadmin_accessFlag_string' in md:
                                    drs2AccessFlags.append(md['object_huldrsadmin_accessFlag_string'])
                                  try: 
                                    info_resp = requests.get(imageUriBase.replace("https","http") + str(md['file_id_num']) + imageInfoSuffix, cookies=cookie)
...
def main(data, document_id, source, host, cookie=None):
        ...
        manifestUriBase = settings.IIIF['manifestUriTmpl'].format(host)
        ...
        # naomi hack: logo = settings.IIIF['logo'] % host
        ...
        if ( (len(drs2ImageWidths) == 0) and (len(drs2ImageHeights) == 0) ):
            # using mock
            drs2ImageWidths, drs2ImageHeights = get_image_dimension_from_solr(document_id)
        ...
        for cvs in canvasInfo:
                ...
                try:
                        ...
                        try:
                                ...
                                response = requests.get(imageUriBase.replace("https","http") + cvs['image'] + imageInfoSuffix, cookies=cookie)
                ...
                if cvs['image'] not in uniqCanvases:
...
if __name__ == "__main__":
        ...
        document_id = sys.argv[2] # manifest identifier
        source = sys.argv[3]      # data source
        outputfile = source + '-' + document_id +  ".json"
        host = sys.argv[4]        # host
